...
**Souvislosti:** [Instanced Actors](instanced-actors.md) · [Přehled algoritmů: wave function collapse](../teorie/algoritmy-prehled.md#algoritmy-ktere-stavi-herni-svety) *(klasika dlaždicové generace mimo PCG framework)* · [Markovovy řetězce](../teorie/markovovy-retezce.md) *(pravděpodobnostní generace)* · [AI agenti: stavba přes PCG](claude-code-ue.md#proc-nativni-mcp-samo-nestaci-a-co-s-tim) *(agent, který místo scény staví graf)* · [Rejstřík: self pruning](../rejstrik.md#self-pruning) · [Rejstřík: PCG](../rejstrik.md#pcg)
